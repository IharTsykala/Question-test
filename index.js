...
class Model {
    ...
    searchCurrentQuestion(levelDifficulty) {
        ...
        // console.log(this.currentListQuestion[this.currentNumberQuestion])      
        // return this.currentListQuestion[this.currentNumberQuestion++]
    }
    nextQuestion() {
        console.log(this.currentListQuestion[this.currentNumberQuestion]) 
        if(this.currentListQuestion[this.currentNumberQuestion]>this.currentListQuestion.length) return
        return this.currentListQuestion[this.currentNumberQuestion++]
    }

    checkCorrectAnswer(collectionInput, nextQuestion) {
        // console.log(collectionInput[nextQuestion.answer].checked)
        this.answerList = []
        this.writeAnswer = 0
        this.falseAnswer = 0
    ...
    getPercentagesWriteAnswer() {
       const percentages =this.writeAnswer/this.currentNumberQuestion*100
...
class View {
    ...
    renderQuestion(fullName, currentQuestion) {
      ...
      renderCurrentQuestion(currentQuestion) {
        ...
        // console.log(this.currentQuestionBlock) 
...
class Controller {
    ...
    startProgram(e) {
       e.preventDefault()            
       const fullName = this.model.startProgram(this.inputFirstName.value, this.inputSecondName.value)
    ...
    nextQuestion() {
        ...
        this.submitButtonAnswer.addEventListener("click", (e) => { 
            this.currentQuestionBlock = wrapper.querySelector(".current-question-block")
            const collectionInput = this.currentQuestionBlock.querySelectorAll('input')
            const nextQuestionObj = this.model.checkCorrectAnswer(collectionInput, nextQuestion)           
            this.nextQuestion()            
        })
    }
